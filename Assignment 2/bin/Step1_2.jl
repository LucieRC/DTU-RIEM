...
#     return system[t,w]*() + (1-system[t,w])*()
# end
...
for w in 1:600
    for t in 1:24
        if system[t,w] == 1
            price_Bal[t,w] = .9*price_DA[t,w]*delta_more[t,w] - 1*price_DA[t,w]*delta_less[t,w]
        end
        if system[t,w] == 0
            price_Bal[t,w] = 1*price_DA[t,w]*delta_more[t,w] - 1.2*price_DA[t,w]*delta_less[t,w]
        end
    end
end


@objective(model, Max, sum(prob * sum(price_DA[t,w]*p_DA[t] + price_Bal[t,w]    
...
# expected_profit = mean(value.(profit))
...
#   the DA market. Then if it produces less than what was scheduled, it can re-purchase the delta at a lower price in the balancing market.
#This behaviour is exactly why the two price scheme got implemented in some balancing markets. 
>>>>>>> 90fbdf9409d794b0221dd5456d3d356f21df1220
