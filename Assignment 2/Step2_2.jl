...
@variable(model, psi_7[n in 1:6, m in 1:6], Bin) #>= 0)
@variable(model, psi_8[n in 1:6, m in 1:6], Bin) #>= 0)

@objective(model, Max, sum(p_i[i]*C_i[i] for i in 1:4) + sum(alpha_bid[k]*d[k] for k in 1:4) - sum(mu_up_k[k]*D_max[k] for k in 1:4) - sum(mu_up_j[j]*P_max_j[j] for j in 1:4) - sum((eta_down_n_m[n,m] + eta_up_n_m[n,m])*get_capacity_line(n,m)*1000 for n in 1:6 for m in get_connected_nodes(n)))
...
# @constraint(model, compl_cst3_5[i in 1:4], 0 <= psi_3[i] <= 1)
...
for n in 1:6, m in get_connected_nodes(n)
    ...
    @constraint(model, eta_down_n_m[n,m] >= 0)
...
# @constraint(model, compl_cst7_1[n in 1:6], 0 <= (get_capacity_line(n,m)*1000 + 50*(theta[n] - theta[m])))
# @constraint(model, compl_cst7_2[n in 1:6], eta_down_n_m[n,m] >= 0)
# @constraint(model, compl_cst7_3[n in 1:6], (get_capacity_line(n,m)*1000 + 50*(theta[n] - theta[m])) <= psi_7[n,m]*M)
# @constraint(model, compl_cst7_4[n in 1:6], eta_down_n_m[n,m] <= (1-psi_7[n,m]*M))
# @constraint(model, compl_cst7_5[n in 1:6], 0 <= psi_7[n,m] <= 1)

for n in 1:6, m in get_connected_nodes(n)
    @constraint(model, 0 <= (get_capacity_line(n,m)*1000 - 50*(theta[n] - theta[m])))
    @constraint(model, eta_up_n_m[n,m] >= 0)
    @constraint(model, (get_capacity_line(n,m)*1000 - 50*(theta[n] - theta[m])) <= psi_8[n,m]*M)
    @constraint(model, eta_up_n_m[n,m] <= (1-psi_8[n,m]*M))
end 
# @constraint(model, compl_cst8_1[n in 1:6], 0 <= 0 <= (get_capacity_line(n,m)*1000 - 50*(theta[n] - theta[m])))
# @constraint(model, compl_cst8_2[n in 1:6], eta_up_n_m[n,m] >= 0)
# @constraint(model, compl_cst8_3[n in 1:6], (get_capacity_line(n,m)*1000 - 50*(theta[n] - theta[m])) <= psi_8[n,m]*M)
# @constraint(model, compl_cst8_4[n in 1:6], eta_up_n_m[n,m] <= (1-psi_8[n,m]*M))
# @constraint(model, compl_cst8_5[n in 1:6], 0 <= psi_8[n,m] <= 1)
