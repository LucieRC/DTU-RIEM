...
@variable(model_step2, power_to_electro[wind_scale, hours_scale] >= 0)
...
@objective(model_step2, Max, sum(sum(bid_price[d]*load_demands[d,h] for d in demands) 
    - sum(offer_price_suppliers[w]*(power_generators[w,h] - power_to_electro[w,h]) for w in wind) 
...
@constraint(model_step2, cst_electrolyzer_prod[w in wind_scale], sum(power_to_electro[w,h] for h in hours_scale)*h2_per_MW >= is_there_electro[w]*total_h2_production)
...
@constraint(model_step2, cst_max_electro_power[w in wind, h in hours], power_to_electro[w,h] <= power_generators[w,h])
