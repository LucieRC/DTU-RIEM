...
## Variables
@variable(model_step2, load_demands[demands_scale] >= 0)
@variable(model_step2, power_suppliers[suppliers_scale] >= 0)
@variable(model_step2, theta[nodes_scale] >= 0)
@variable(model_step2, power_to_electro_from_wind[electrolyzers] >= 0) #power from the wind the electrolyzer is installed on


## Objective
@objective(model_step2, Max, sum(bid_price[d]*load_demands[d] for d in demands)     
# - sum(offer_price_suppliers[e]*(power_suppliers[e,h] - power_to_electro_from_wind[e,h]) for e in electrolyzers)
- sum(offer_price_suppliers[s]*power_suppliers[s] for s in suppliers))

# Constraints
@constraint(model_step2, cst_demands[d in demands], load_demands[d] <= max_load_demands[d,chosen_hour])
@constraint(model_step2, cst_suppliers[s in suppliers], power_suppliers[s] <= max_power_suppliers[s,chosen_hour])
@constraint(model_step2, cst_electrolyzer_prod[e in electrolyzers],    
    power_to_electro_from_wind[e]*h2_per_MW >= total_h2_production)
@constraint(model_step2, cst_max_electro_power1[e in electrolyzers],  
    power_to_electro_from_wind[e] <= power_suppliers[e])
@constraint(model_step2, cst_max_electro_power2[e in electrolyzers],  
    power_to_electro_from_wind[e] <= max_power_production_wind_farm[e]/2)


for n in nodes
    for m in get_connected_nodes(n)
        @constraint(model_step2, -get_capacity_line(n,m) <= get_susceptance_line(n,m)*(theta[n] - theta[m]) <= get_capacity_line(n,m))
    end
end

@constraint(model_step2, cst_nodes[n in nodes], 
    - sum(power_suppliers[s] for s in [connected_conventional[n];connected_wind[n]])
    + sum(load_demands[d] for d in connected_demands[n])
    + sum(power_to_electro_from_wind[e] for e in connected_electrolyzers[n])
...
CSV.write("created_data/LUCIE_17.03_Step3_nodal_DA_MCP_24h.csv", Tables.table(DA_MCP_24h))
DA_production_suppliers_24h = value.(power_suppliers)
CSV.write("created_data/Step3_nodal_DA_production_suppliers_24h.csv", Tables.table(DA_production_suppliers_24h))
DA_demands_24h = value.(load_demands)
CSV.write("created_data/Step3_nodal_DA_demands_24h.csv", Tables.table(DA_demands_24h))
DA_thetas = value.(theta)
CSV.write("created_data/Step3_nodal_DA_thetas_24h.csv", Tables.table(DA_thetas))
DA_eletro = value.(power_to_electro_from_wind)
CSV.write("created_data/Step3_nodal_DA_electro_24h.csv", Tables.table(DA_electro))
