...
for w in winds, hour in hours
    forecast_std_error[w,hour] = 0.3*sum(DA_production_suppliers_24h[w,hour]) #carefull -> per hour
end
...
@variable(model_step4_B, delta_theta[n in nodes, h in hours])
@variable(model_step4_B, wind_spillage[h in hours] >= 0)
@variable(model_step4_B, load_curtailment[d in demands, h in hours] >= 0)

## Objective
@objective(model_step4_B, Min, sum(sum((up_cost_electro[e,h]*up_prod[e,h] - down_cost_electro[e,h]*down_prod[e,h]) for e in electrolyzers)    
    + cost_curtailment*sum(load_curtailment[d,h] for d in demands) for h in hours))

# Constraints
@constraint(model_step4_B, cst_down_prod[e in electro, h in hours], down_prod[e,h] <= down_bound_prod[e,h])
@constraint(model_step4_B, cst_up_prod[e in electro, h in hours], up_prod[e,h] <= up_bound_prod[e,h])
@constraint(model_step4_B, cst_load_curt[d in demands, h in hours], load_curtailment[d,h] <= DA_demands_24h[d,h])
...
@constraint(model_step4_B, cst_theta[n in nodes, h in hours], -pi <= DA_thetas_24h[n,h] + delta_theta[n,h] <= pi)
...
#     + sum((DA_demands_24h[d,h] - load_curtailment[d,h]) for d in connected_demands[n])     
#     + sum(forecast_eps[w,h] for w in connected_wind[n])
#     + sum(get_susceptance_line(n,m)*((DA_thetas_24h[n,h]+delta_theta[n,h]) - (DA_thetas_24h[m,h]+delta_theta[m,h])) for m in connected_nodes[n]) == 0)

@constraint(model_step4_B, market_equilibrium[n in nodes, h in hours],    
    sum(DA_electro_24h[e,h] + down_prod[e,h] - up_prod[e,h] for e in connected_electrolyzers[n])   
    - sum(DA_production_suppliers_24h[w,h] - wind_spillage[w,h] for w in connected_wind[n])
    - sum(DA_production_suppliers_24h[c,h] connected_conventional[n])  
    + sum((DA_demands_24h[d,h] - load_curtailment[d,h]) for d in connected_demands[n])     
    + sum(forecast_eps[w,h] for w in connected_wind[n])
    + sum(get_susceptance_line(n,m)*((DA_thetas_24h[n,h]+delta_theta[n,h]) - (DA_thetas_24h[m,h]+delta_theta[m,h])) for m in connected_nodes[n]) == 0)


# @constraint(model_step4_B, market_eq[n in nodes, h in hours],
