...
- sum(offer_price_suppliers[e]*(power_generators[e,h] - power_to_electro_from_wind[e,h]) for e in electrolyzers)
- sum(offer_price_suppliers[s]*power_generators[s,h] for s in collect(3:18)) for h in hours))
...
@constraint(model_step2, cst_equilibrium[h in hours], sum(load_demands[d,h] for d in demands) + sum(power_generators[e,h] - power_to_electro_from_wind[e,h] for e in electrolyzers) - sum(power_generators[s,h] for s in collect(3:18)) == 0)
