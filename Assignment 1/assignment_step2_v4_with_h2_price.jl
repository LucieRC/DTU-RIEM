...
# @variable(step2_3, is_there_electro[wind_scale] >= 0, Bin)
...
# @constraint(step2_3, cst_equilibrium[h in hours], sum(load_demands[d,h] for d in demands) - sum(offer_price_suppliers[e]*(power_generators[e,h] - power_to_electro[e,h]) for e in electro) - sum(offer_price_suppliers[s]*power_generators[s,h] for s in collect(3:18)) == 0) # modify
...
# @constraint(step2_3, cst_electrolyzer_prod[w in wind], sum(is_there_electro[w]*(power_generators[w,h]<=power_electrolyzer[w,h] ? power_electrolyzer[w] : power_generators[w]/2) for h in hours_scale)*h2_per_MW >= is_there_electro[w]*total_h2_production)
# @constraint(step2_3, cst_electrolyzer_prod[e in electro], sum(power_to_electro[e,h] for h in hours_scale)*h2_per_MW >= total_h2_production)
...
#@constraint(step2_3, cst_max_electro_power[e in electro, h in hours], power_to_electro[e,h] <= power_generators[e,h])
@constraint(step2_3, cst_max_electro_power[w in wind, h in hours], power_to_electro_from_wind[w,h] <= power_generators[w,h])
# @constraint(step2_3, cst_nb_electro, sum(is_there_electro[w] for w in wind) >= 2)
...
# println("Social welfare: ", objective_value(step2_3))
...
# println("Objective value: ", objective_value(step2_3))
