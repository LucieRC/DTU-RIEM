...
@constraint(model_step2, cst_nodes[n in nodes, h in hours],
    ...
    + sum(power_to_electro_from_wind[e,h] for e in connected_electrolyzers[n]) == 0)
...
CSV.write("created_data/Step4.2.1_nodal_DA_MCP_24h.csv", Tables.table(DA_MCP_24h))
DA_production_suppliers_24h = value.(power_suppliers)
CSV.write("created_data/Step4.2.1_nodal_DA_production_suppliers_24h.csv", Tables.table(DA_production_suppliers_24h))
DA_demands_24h = value.(load_demands)
CSV.write("created_data/Step4.2.1_nodal_DA_demands_24h.csv", Tables.table(DA_demands_24h))
...
CSV.write("created_data/Step4.2.1_nodal_DA_electro_24h.csv", Tables.table(DA_electro))
