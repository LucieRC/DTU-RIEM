...
- sum(offer_price_suppliers[s]*power_suppliers[s,h] for s in collect(3:18))
+ sum(power_to_electro_from_wind[e,h] for e in electrolyzers)*h2_per_MW*price_per_h2_kg for h in hours))

# Constraints
@constraint(model_step2, cst_demands[d in demands, h in hours], load_demands[d,h] <= max_load_demands[d,h])
@constraint(model_step2, cst_generators[s in suppliers, h in hours], power_suppliers[s,h] <= max_power_suppliers[s,h])
@constraint(model_step2, cst_equilibrium[h in hours], sum(load_demands[d,h] for d in demands) + sum(power_suppliers[e,h] - power_to_electro_from_wind[e,h] for e in electrolyzers) - sum(power_suppliers[s,h] for s in collect(3:18)) == 0)
...
@constraint(model_step2, cst_max_electro_power[e in electrolyzers, h in hours], power_to_electro_from_wind[e,h] <= power_suppliers[e,h])
...
CSV.write("created_data/Step2.2_DA_MCP_24h.csv", Tables.table(DA_MCP_24h))
...
# power_suppliers #18*24 matrix
# println("p_g = ", value.(power_suppliers))

# value.(power_to_electro_from_wind)

#new supply
# value.(power_suppliers) .- vcat(value.(power_to_electro_from_wind), zeros(12,24))
# # add 18-6 lines to power_to_electro_from_wind

# value.(power_suppliers)
# CSV.write("DA_production_suppliers_24h.csv", Tables.table(power_suppliers))
