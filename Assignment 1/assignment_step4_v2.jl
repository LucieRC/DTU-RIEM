...
model_step4_B = Model(Gurobi.Optimizer)
...
# Results of DA model
DA_MCP_24h = Matrix(CSV.read("created_data/Step3_nodal_DA_MCP_24h.csv", DataFrame, delim=","))
DA_production_suppliers_24h = Matrix(CSV.read("created_data/Step3_nodal_DA_production_suppliers_24h.csv", DataFrame, delim=","))
DA_demands_24h = Matrix(CSV.read("created_data/Step3_nodal_DA_demands_24h.csv", DataFrame, delim=","))
DA_thetas_24h = Matrix(CSV.read("created_data/Step3_nodal_DA_thetas_24h.csv", DataFrame, delim=","))
DA_electro_24h = Matrix(CSV.read("created_data/Step3_nodal_DA_electro_24h.csv", DataFrame, delim=",")[1:2,1:24])
...
# max_electrolyzers = max_power_production_wind_farm./2[1]
max_electrolyzers = [250 250]

# New parameters
down_cost_electro = 0.1*DA_MCP_24h
up_cost_electro = 0.15*DA_MCP_24h
...
# down_bound_prod = DA_production_suppliers_24h #12-elt vector
# up_bound_prod = max_electrolyzers - DA_production_suppliers_24h
...
## Variables
@variable(model_step4_B, down_prod[e in electro, h in hours] >= 0)
@variable(model_step4_B, up_prod[e in electro, h in hours] >= 0)
@variable(model_step4_B, delta_theta[n in nodes, h in hours] >= 0)

## Objective
@objective(model_step4_B, Min, sum(up_cost_electro[h]*up_prod[e,h]   
- down_cost_electro[h]*down_prod[e,h] for e in electrolyzers for h in hours))

# Constraints
@constraint(model_step4_B, cst_demands[e in electro, h in hours], down_prod[e,h] <= down_bound_prod[e,h])
@constraint(model_step4_B, cst_suppliers[e in electro, h in hours], up_prod[e,h] <= up_bound_prod[e,h])

@constraint(model_step4_B, cst_electrolyzer_prod[e in electrolyzers],
...
for n in nodes
    for m in get_connected_nodes(n)
        @constraint(model_step4_B, [h in hours],     
...
@constraint(model_step4_B, market_equilibrium[n in nodes, h in hours],    
    sum(up_prod[e,h] - down_prod[e,h] for e in connected_electrolyzers[n])     
    - sum(DA_production_suppliers_24h[s,h] for s in [connected_wind[n]; connected_conventional[n]])  
    + sum(DA_demands_24h[d,h] for d in connected_demands[n])     
