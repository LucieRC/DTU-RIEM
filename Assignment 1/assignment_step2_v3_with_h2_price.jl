...
## Objective
@objective(model_step2, Max, sum(sum(bid_price[d]*load_demands[d,h] for d in demands) - sum(offer_price_suppliers[e]*(power_generators[e,h] - power_to_electro[e,h]) for e in electro) - sum(offer_price_suppliers[s]*power_generators[s,h] for s in collect(3:18)) + sum(power_to_electro[e,h] for e in electro)*h2_per_MW*price_per_h2_kg for h in hours))
#it is ok in terms of unit
