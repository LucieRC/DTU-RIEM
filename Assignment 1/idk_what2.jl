...
load_curtailment_cost = 500 #$/MWh
h2_percent_cost_reduced = 0.1
h2_percent_cost = 0.15
...
@variable(model_step2, power_to_electro_from_wind[wind_scale, hours_scale] >= 0)
# @variable(model_step2, power_to_electro_from_other[wind_scale, hours_scale] >= 0)


## Objective
@objective(model_step2, Max, sum(sum(bid_price[d]*load_demands[d,h] for d in demands)   
    - sum(offer_price_suppliers[w]*(power_suppliers[w,h] - power_to_electro_from_wind[w,h]) for w in wind)
    - sum(offer_price_suppliers[s]*power_generators[s,h] for s in collect(7:18)) for h in hours))
...
@constraint(model_step2, cst_electrolyzer_prod[s in suppliers_scale], sum(power_to_electro_from_wind[w,h] for h in hours_scale)*h2_per_MW >= is_there_electro[w]*total_h2_production)
