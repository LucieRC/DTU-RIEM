...
# @constraint(model_step2, transmission_capacity[n in nodes, m in nodes, h in hours], 
#     (if m in get_connected_nodes(n) ? (-get_capacity_line(n,m) <= get_susceptance_line(n,m)*(theta[n] - theta[m]) <= get_capacity_line(n,m)) : 0) )
# @constraint(model_step2, transmission_capacity[n in nodes, m in nodes, h in hours],     
#     (-get_capacity_line(n,m) ? -get_capacity_line(n,m) : 0) <= get_susceptance_line(n,m)*(theta[n] - theta[m]) <= (get_capacity_line(n,m) ? get_capacity_line(n,m) : 0))
# @constraint(model_step2, transmission_capacity[n in nodes, m in get_connected_nodes(n), h in hours],     
#     -get_capacity_line(n,m) <= get_susceptance_line(n,m)*(theta[n] - theta[m]) <= get_capacity_line(n,m))
...
@constraint(model_step2, cst_max_electro_power1[e in electrolyzers, h in hours],  
    power_to_electro_from_wind[e,h] <= power_suppliers[e,h])
@constraint(model_step2, cst_max_electro_power2[e in electrolyzers, h in hours],  
    power_to_electro_from_wind[e,h] <= max_power_production_wind_farm[e]/2)
@constraint(model_step2, cst_one_node_to_zero[h in hours], theta[24,h]==0)
...
@constraint(model_step2, cst_nodes[n in nodes, h in hours],    
    # - sum(power_suppliers[e,h]-power_to_electro_from_wind[e,h] for e in connected_electrolyzers[n])
